...
class MyProfileView(APIView):
    ...
    authentication_classes = [JWTAuthentication]
    def get(self,request):
        serializer=UserSerializerWithFilters(request.user)
        return renderResponse(data=serializer.data,message="My Profile",status=200)
...
class UpdateUsers(generics.UpdateAPIView):
    ...
    def get_queryset(self):
        return Users.objects.filter(id=self.kwargs['pk'])
