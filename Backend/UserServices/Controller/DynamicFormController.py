...
class DynamicFormController(APIView):
    ...
    def post(self,request,modelName,id=None):
        ...
        required_fields=[field.name for field in fields_info if not field.null and field.default == NOT_PROVIDED and field.name not in exclude_fields and field.get_internal_type() != 'JSONField']
        ...
        for field in fields_info:
            if field.is_relation and field.name in fieldsdata:
                val = fieldsdata[field.name]
                if isinstance(val, int) or (isinstance(val, str) and val.isdigit()):
                    related_model=field.related_model
                    try:
                        fieldsdata[field.name]=related_model.objects.get(id=int(val))
            ...
            elif field.get_internal_type() == 'JSONField' and field.name not in fieldsdata:
        ...
        if id:
            model_instace=model_class.objects.filter(id=id)
    ...
    def get(self,request,modelName,id=None):
        ...
        if id:
            model_instance=model_class.objects.filter(id=id)
