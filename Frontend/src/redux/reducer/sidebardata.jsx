...
export const fetchSidebar=createAsyncThunk('data/fetchSidebar',async()=>{
    ...
    const setActiveAndExpanded=(item)=>{
        ...
        const isRootMatch = item.module_url === '/' && (pathname === '/' || pathname === '/home');
        const isPathMatch = item.module_url !== '/' && item.module_url && pathname.indexOf(item.module_url) !== -1;
        
        if(isRootMatch || isPathMatch){
            item.active=true;
            item.expanded=true;
            return true;
        }
    ...
    reducers:{
        ...
        triggerPageChange(state,action){
            state.items.forEach(item=>{
                ...
                item.submenus?.forEach(submenu=>{
                    submenu.active=false;
                    const isSubRootMatch = submenu.module_url === '/' && (pathname === '/' || pathname === '/home');
                    const isSubPathMatch = submenu.module_url !== '/' && submenu.module_url && pathname.indexOf(submenu.module_url)!==-1;
                    if(isSubRootMatch || isSubPathMatch){
                        submenu.active=true;
                        item.active=true;
                        item.expanded=true;
                    }
                ...
                const isRootMatch = item.module_url === '/' && (pathname === '/' || pathname === '/home');
                const isPathMatch = item.module_url !== '/' && item.module_url && pathname.indexOf(item.module_url)!==-1;
                if((isRootMatch || isPathMatch) && (!item.submenus || item.submenus.length===0)){
                    item.active=true;
                    item.expanded=true;
                }
            });
