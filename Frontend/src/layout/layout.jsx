...
import { CssBaseline, ThemeProvider, createTheme, AppBar, Toolbar, IconButton, Typography, Drawer, List, ListItem, ListItemButton, ListItemText, Collapse, Divider, Card, CardContent, Fab, Box, Hidden, InputBase, Avatar, Menu, MenuItem, ListItemIcon, BottomNavigation, BottomNavigationAction, Link, Breadcrumbs } from '@mui/material';
import { LightMode, DarkMode, Menu as MenuIcon, ExpandLess, ExpandMore, Search as SearchIcon, AccountCircle, Settings as SettingsIcon, Notifications as NotificationsIcon, Logout, Home, Code as CodeIcon, Public as PublicIcon, Business as BusinessIcon, AlternateEmail as AlternateEmailIcon, AutoAwesomeTwoTone, Circle, AddCircleOutlineOutlined, DashboardOutlined, ShoppingCartOutlined, StorefrontOutlined, GroupOutlined, InventoryOutlined, CategoryOutlined, Category, ShoppingBasketOutlined, ShoppingBasketRounded, ReceiptOutlined, WarehouseOutlined, InsertChartOutlined } from '@mui/icons-material';
...
const Layout = ({sidebarList,pageTitle,childPage}) => {
  ...
  const drawer = (
    <div
      ...
      className='sidebar'
    >
      <Box sx={{ display: 'flex', alignItems: 'center', height: 40, mt: 1.5, mb: 3.5, ml: -3.5 }}>
        <Box component="img" src={newLogo} alt="Nexora Logo" sx={{ height: 160, objectFit: 'contain', pointerEvents: 'none' }} />
      </Box>
      <List sx={{ px: 1 }}>
        {sidebarItems.map((sidebarItem) => (
            <React.Fragment key={sidebarItem.id}>
            <ListItemButton id={`onboarding-sidebar-${sidebarItem.module_name.toLowerCase().replace(/\s+/g, '-')}`} onClick={()=>handleSidebarMenuClick(sidebarItem)} selected={sidebarItem?.active && sidebarItem.submenus.length===0}>
                <ListItemIcon sx={{ minWidth: 40 }}>
                    {getIcon(sidebarItem.module_icon)}
                </ListItemIcon>
                <ListItemText primary={sidebarItem.module_name} primaryTypographyProps={{ fontWeight: sidebarItem?.active ? 600 : 500 }} />
            ...
            {"submenus" in sidebarItem && sidebarItem.submenus.length>0 && (
              ...
              {sidebarItem.submenus.map(child => (
                            <ListItemButton key={child.module_name} onClick={()=>handleSidebarMenuClick(child)} selected={child?.active}>
                                <ListItemIcon sx={{ minWidth: 40 }}>
                                    {getIcon(child.module_icon)}
                                </ListItemIcon>
                                <ListItemText primary={child.module_name} primaryTypographyProps={{ fontWeight: child?.active ? 600 : 500, fontSize: 13 }} />
                            </ListItemButton>
                ))}
                </List>
                </Collapse>              
            ...
            <AppBar position="sticky" sx={{ backgroundColor: theme.palette.background.default, backgroundImage: 'none', borderBottomWidth: '1px', borderBottomStyle: 'solid', borderBottomColor: theme.palette.background.paper }} className='appbar'>
              <Toolbar sx={{ px: 3 }}>
                ...
                <Breadcrumbs aria-label="breadcrumb" sx={{ flexGrow: 1 }}>
                    <Typography variant="body1" color="text.secondary">Workspace</Typography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  >
                    <Avatar sx={{ width: 28, height: 28, bgcolor: 'primary.main', fontSize: 14 }}>M</Avatar>
                  </IconButton>
                </Box>
              </Toolbar>
            </AppBar>
