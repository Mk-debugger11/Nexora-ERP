...
const GlobalManageReviews = () => {
    ...
    const getReviews=async()=>{
        ...
        const result=await callApi({url:`products/reviews/`,method:'GET',params:{
    ...
    const toggleStatus=async(id,status)=>{
        const result=await callApi({url:`products/reviews/${id}/`,method:'PATCH',body:{status:status}});
        if(result){
            getReviews();
        }
    }
    ...
    return (
        <Box component={"div"} sx={{width:'100%'}}>
            <Box display="flex" justifyContent="space-between" mb={2}>
                <Typography variant="h5" mb={2}>Global Product Reviews </Typography>
            </Box>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Breadcrumbs>
                    <Typography variant="body2" onClick={() => navigate('/')} sx={{ cursor: 'pointer' }}>Home</Typography>
                    <Typography variant="body2" sx={{ cursor: 'pointer' }}>Manage Global Reviews</Typography>
                </Breadcrumbs>
            </Box>
...
export default GlobalManageReviews