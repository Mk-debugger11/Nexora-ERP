...
import { DialogTitle, DialogContentText, DialogActions, Chip } from "@mui/material";
...
const ManageProducts = ({onProductSelected}) => {
    ...
    const confirmDelete = async () => {
        ...
        const result = await callApi({ url: `products/delete/${productToDelete.id}/`, method: 'DELETE' });
        if(result){
    ...
    const onToggleStatus = async (params) => {
        const result = await callApi({ url: `products/toggle-status/${params.row.id}/`, method: 'PUT' });
        if(result){
    ...
    const generateColumns=(data)=>{
        if(data.length>0){
            let columns=[{field:'action',headerName:'Action',width:220,sortable:false,renderCell:(params)=>{
                return <>
                    <IconButton onClick={()=>onViewClick(params)}>
            ...
            const allowedFields = [
                ...
                { field: 'initial_buying_price', headerName: 'Purchase Price', width: 150 },
                { field: 'initial_selling_price', headerName: 'Selling Price', width: 150 },
            ...
            allowedFields.forEach(col => {
                if(col.field === 'image') {
                    columns.push({field: col.field, headerName: col.headerName, width: col.width, sortable: false, renderCell: (params) => {
                    ...
                    columns.push({field: col.field, headerName: col.headerName, width: col.width, renderCell: (params) => {
                        return <Chip size="small" label={params.row.status} color={params.row.status === 'ACTIVE' ? 'success' : 'default'} />
                    }});
                } else {
                    columns.push({field: col.field, headerName: col.headerName, width: col.width});
                }
            });
        ...
        <Box component={"div"} sx={{width:'100%'}}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Breadcrumbs>
                    ...
                    <Typography variant="body2" onClick={()=>navigate('/manage/product')} sx={{cursor:'pointer'}}>Manage Products</Typography>
                </Breadcrumbs>
                <Button variant="contained" onClick={()=>navigate('/form/product')} startIcon={<Add />}>Add Product</Button>
            </Box>}
                ...
                <Dialog open={deleteDialogOpen} onClose={cancelDelete}>
                    ...
                    <DialogContent>
                        <DialogContentText>
                            ...
                            This action cannot be undone and will be blocked if active inventory exists.
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions>
