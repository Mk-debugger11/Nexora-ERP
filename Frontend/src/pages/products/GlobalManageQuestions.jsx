...
const GlobalManageQuestions = () => {
    ...
    const getQuestions=async()=>{
        ...
        const result=await callApi({url:`products/questions/`,method:'GET',params:{
    ...
    const toggleStatus=async(id,status)=>{
        const result=await callApi({url:`products/questions/${id}/`,method:'PATCH',body:{status:status}});
        if(result){
            getQuestions();
        }
    }
    ...
    return (
        <Box component={"div"} sx={{width:'100%'}}>
            <Box display="flex" justifyContent="space-between" mb={2}>
                <Typography variant="h5" mb={2}>Global Product Questions </Typography>
            </Box>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Breadcrumbs>
                    <Typography variant="body2" onClick={() => navigate('/')} sx={{ cursor: 'pointer' }}>Home</Typography>
                    <Typography variant="body2" sx={{ cursor: 'pointer' }}>Manage Global Questions</Typography>
                </Breadcrumbs>
            </Box>
            <Grid container spacing={2}>
                <Grid item xs={12} sm={showAddQuestions?8:12} lg={showAddQuestions?9:12}>
            <Box display="flex" justifyContent="space-between" alignItems={"center"} mb={2}>
...
export default GlobalManageQuestions