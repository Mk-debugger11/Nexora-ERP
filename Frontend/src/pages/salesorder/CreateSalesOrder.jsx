...
const CreateSalesOrder = () => {
  ...
  const {id}=useParams();
  const [soFields, setPoFields] = useState([]);
  const [soItemFields, setPoItemFields] = useState([]);
  ...
  const getFormFields = async () => {
    const idVar= id ? id+"/" : '';
    const response = await callApi({ url: `orders/salesOrder/${idVar}` });
    if (response && response.status === 200) {
      setPoFields(response.data.data.soFields);
      setPoItemFields(response.data.data.soItemFields);
      setSupplierEmail(response.data.data?.soData?.supplier_email);
      setSupplierId(response.data.data?.soData?.supplier_id);
      methods.setValue('supplier_id',response.data.data?.soData?.supplier_id);
      methods.setValue('items',response.data.data?.soItems);
    }
  };
    ...
    return methods?.watch('items')?.map((item, index) => (
      <TableRow>
        ...
        {fieldType.map((fieldT, index2) =>
          soItemFields?.[fieldT].map((field, index3) => {
            let tempField = { ...field };
  ...
  const onSubmit=async(data)=>{
    ...
    const idVar= id ? id+"/" : '';
    const response = await callApi({ url: `orders/salesOrder/${idVar}`,method:'POST',body:data});
        ...
        <form>
          <Box display="flex" justifyContent={"space-between"}>
            <Breadcrumbs>
              ...
              <Typography variant="body2">Create Sales Order</Typography>
            </Breadcrumbs>
          </Box>
          ...
          <Grid container spacing={2} mt={2} mb={2}>
            {fieldType?.map((field, index) =>
              soFields?.[field]?.map((field1, index1) =>
          ...
          <Box display={"flex"} justifyContent={"space-between"}>
            <Typography variant="h6">Sales Order Products</Typography>
            <Button
              variant="contained"
              startIcon={<Add />}
              color="primary"
              ...
              <TableHead>
                <TableRow>
                  ...
                  {fieldType.map((item, index) =>
                    soItemFields?.[item]?.map((field, index2) => (
