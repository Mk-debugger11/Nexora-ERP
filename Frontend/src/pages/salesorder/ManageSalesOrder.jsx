...
const ManageSalesOrder = ({onProductSelected}) => {
    ...
    const [soToDelete, setPoToDelete] = useState(null);
    ...
    const getProducts=async()=>{
        ...
        const result=await callApi({url:'orders/salesOrderList/',method:'GET',params:{
    ...
    const onDeleteClick=(params)=>{
        setPoToDelete(params.row);
        setDeleteDialogOpen(true);
    }
    const confirmDelete = async () => {
        if (!soToDelete) return;
        const res = await callApi({ url: `orders/salesOrder/delete/${soToDelete.id}/`, method: 'DELETE' });
        if (res) {
    ...
    const onEditClick=(params)=>{
        console.log(params);
        navigate(`/create/so/${params.row.id}`)
    }
        ...
        if(data.length>0){
            let columns=[{field:'action',headerName:'Action',width:180,sortable:false,renderCell:(params)=>{
                return <>
                    ...
                    <IconButton onClick={()=>navigate(`/so/details/${params.row.id}`)}>
                        <Dashboard color="primary" />
                    </IconButton>
                    }
                    { (params.row.status === 'DRAFT' || params.row.status === 'CREATED') && (
    ...
    return (
        <Box component={"div"} sx={{width:'100%'}}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <Button variant="contained" onClick={()=>navigate('/create/po')} startIcon={<Add />}>Add Sales Order</Button>
            </Box>}
            ...
            <DataGrid
                ...
                <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                    ...
                    <DialogContent>
                        <DialogContentText>
                            Are you sure you want to delete Sales Order #{soToDelete?.so_code}?
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions>
