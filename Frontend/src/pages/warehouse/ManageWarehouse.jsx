...
import { Box, Breadcrumbs, Button, Chip, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Divider, Grid, IconButton, LinearProgress, Switch, Table, TextField, Typography } from "@mui/material";
...
const ManageWarhouse = () => {
    ...
    const [deleteType, setDeleteType] = useState('warehouse');
    ...
    const onDeleteRackShelf=(item)=>{
        setItemToDelete(item);
        setDeleteType('rackshelf');
        setDeleteDialogOpen(true);
    }
    const confirmDelete = async () => {
        if(!itemToDelete) return;
        const url = deleteType === 'warehouse' 
            ? `inventory/deleteWarehouse/${itemToDelete.id}/`
            : `inventory/deleteRackShelfFloor/${itemToDelete.id}/`;
        const result = await callApi({ url, method: 'DELETE' });
        if(result){
            toast.success(deleteType === 'warehouse' ? 'Warehouse deleted' : 'Rack/Shelf deleted');
            getWarehouse();
        }
    ...
    const generateColumns=(data)=>{
        if(data.length>0){
            let columns=[{field:'action',headerName:'Action',width:220,sortable:false,renderCell:(params)=>{
                return <>
            ...
            for(const key in data[0]){
                if(key==='rack_shelf_floor'){
                    ...
                    columns.push({field:"status",headerName:"Status",width:130,sortable:false,renderCell:(params)=>{
                        return <Chip size="small" label={params.row.status} color={params.row.status==='ACTIVE'?'success':'default'} onClick={()=>toggleStatus(params.row.id, params.row.status==='ACTIVE'?'INACTIVE':'ACTIVE')} sx={{cursor:'pointer'}} />
                    }})
                }
                else if(['domain_user_id','added_by_user_id'].includes(key)){
    ...
    return (
        <Box component={"div"} sx={{width:'100%'}}>
                ...
                {showViewRackAndShelf && <Grid item xs={12} sm={5} lg={4} sx={{height:'600px',overflowY:'auto'}} ref={divImage}>
                        ...
                        <Divider/>
                        {
                            selectedRackAndShelfList.map((item,index)=>(
                                <RackAndShelfCard key={index} item={item} onEditClick={onEditClickRackAndShelf} onDeleteClick={onDeleteRackShelf}/>
                            ))
                        }
            ...
            <Dialog open={deleteDialogOpen} onClose={cancelDelete}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Are you sure you want to delete {deleteType === 'warehouse' ? `warehouse "${itemToDelete?.name}"` : `rack/shelf "${itemToDelete?.name}"`}? 
